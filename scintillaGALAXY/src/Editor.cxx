...
void Editor::InvalidateSelection(SelectionRange newMain, bool invalidateWholeSelection) {
	...
	// by midi
	if( vs.showCaretLineBackground )
	{
		int currentLine = pdoc->LineFromPosition(sel.RangeMain().caret.Position());
		int newLine = pdoc->LineFromPosition(newMain.caret.Position());
...
void Editor::PaintSelMargin(Surface *surfWindow, PRectangle &rc) {
	...
	// by midi
	const int nCaretSubLineStyle = STYLE_LASTPREDEFINED + 4;
	bool hasCaretLineDrawn = false;
	PRectangle rcCaretLine;
	...
	for (int margin = 0; margin < vs.margins; margin++) {
		if (vs.ms[margin].width > 0) {
			...
			if (vs.ms[margin].style != SC_MARGIN_NUMBER) {
				if (vs.ms[margin].mask & SC_MASK_FOLDERS)
					...
					case SC_MARGIN_FORE:
						...
						break;
					case SC_MARGIN_APPSTYLED0:
					case SC_MARGIN_APPSTYLED1:
					case SC_MARGIN_APPSTYLED2:
					case SC_MARGIN_APPSTYLED3:
						colour = vs.styles[ STYLE_LASTPREDEFINED+vs.ms[margin].style-SC_MARGIN_APPSTYLED_FIRST ].back;
						break;
					default:
			...
			// by midi
			int caretLine = pdoc->LineFromPosition( sel.IsRectangular() ? sel.Rectangular().caret.Position() : sel.MainCaret() );
			int caretSubLineFirst = cs.DisplayFromDoc(caretLine);
			int caretSubLineLast = cs.DisplayFromDoc(caretLine+1);
			...
			while ((visibleLine < cs.LinesDisplayed()) && yposScreen < rcMargin.bottom) {
				...
				// by midi
				bool isCaretSubLine = vs.showCaretLineBackground && caret.active && visibleLine>=caretSubLineFirst && visibleLine<caretSubLineLast;
				if( isCaretSubLine )
				{
					if( !hasCaretLineDrawn )
					{
						hasCaretLineDrawn = true;
						rcCaretLine = rcMarker;
						rcCaretLine.top = rcMarker.top;
					}
					rcCaretLine.bottom = rcMarker.bottom;
					surface->FillRectangle(rcMarker, vs.styles[nCaretSubLineStyle].back);
				}
				...
				if (vs.ms[margin].style == SC_MARGIN_NUMBER) {
					if (firstSubLine) {
						...
						surface->DrawTextNoClip(rcNumber, vs.styles[STYLE_LINENUMBER].font,
								...
								isCaretSubLine ? vs.styles[nCaretSubLineStyle].back : vs.styles[STYLE_LINENUMBER].back);
	...
	if( hasCaretLineDrawn )
	{
		rcBlankMargin.top = rcCaretLine.top;
		rcBlankMargin.bottom = rcCaretLine.bottom;
		surface->FillRectangle(rcBlankMargin, vs.caretLineBackground);
	}
...
void Editor::Paint(Surface *surfaceWindow, PRectangle rcArea) {
	...
	const int leftTextOverlap = 0; // by midi, i dont allow kkk
