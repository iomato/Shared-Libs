...
namespace aux 
{
  ...
  public:
    ...
    explicit w2a(slice<wchar_t> s):buffer(0), n(0)
    {
  ...
  class w2oem 
  {
    ...
    void init(const wchar_t* wstr, unsigned int nu)
    { 
      n = WideCharToMultiByte(CP_OEMCP,0,wstr,nu,0,0,0,0);
      buffer = (n < (64-1))? local : new char[n+1];
      WideCharToMultiByte(CP_OEMCP,0,wstr,nu,buffer,n,0,0);
      buffer[n] = 0;
    }
  public:
    explicit w2oem(const wchar_t* wstr):buffer(0),n(0)
    { 
      if(wstr)
        init(wstr,(unsigned int)wcslen(wstr));
    }
    ...
    explicit w2oem(slice<wchar_t> s);
