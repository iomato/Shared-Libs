...
EXTERN_C UINT VALAPI ValueInvoke( VALUE* pval, VALUE* pthis, UINT argc, const VALUE* argv, VALUE* pretval, LPCWSTR url);
  ...
  namespace json
  {
    ...
    public: 
      ...
      bool operator == (const string_t& rs) const { return aux::slice<TC>(c_str(), length()) == rs; }
      bool operator == (aux::slice<TC> rs) const { return aux::slice<TC>(c_str(), length()) == rs; }
      bool operator == (const TC* rs) const { return chars() == aux::chars_of(rs); }
      bool operator != (const string_t& rs) const { return aux::slice<TC>(c_str(), length()) != rs; }
      bool operator != (aux::slice<TC> rs) const { return chars() != rs; }
      bool operator != (const TC* rs) const { return chars() != aux::chars_of(rs); }
    };
    ...
    public:
     ...
     ~value()                 { ValueClear(this); }
      ...
      value( aux::wchars ws )   { ValueInit(this); ValueStringDataSet(this, ws.start, ws.length, 0); }
